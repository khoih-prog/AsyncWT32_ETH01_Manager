...
* [Why do we need this AsyncWT32_ETH01_Manager library](#why-do-we-need-this-async-AsyncWT32_ETH01_Manager-library)
  * [Features](#features)
...
* [Debug Terminal Output Samples](#debug-terminal-output-samples)
  * [1. Async_ConfigOnDoubleReset using LittleFS on ESP32_DEV with ETH_PHY_LAN8720](#1-Async_ConfigOnDoubleReset-using-LittleFS-on-ESP32_DEV-with-ETH_PHY_LAN8720)
  * [2. Async_ConfigOnSwichFS using LittleFS on ESP32_DEV with ETH_PHY_LAN8720](#2-Async_ConfigOnSwichFS-using-LittleFS-on-ESP32_DEV-with-ETH_PHY_LAN8720)
  * [3. Async_ESP32_FSWebServer_DRD using LittleFS on ESP32_DEV with ETH_PHY_LAN8720](#3-Async_ESP32_FSWebServer_DRD-using-LittleFS-on-ESP32_DEV-with-ETH_PHY_LAN8720)
* [Debug](#debug)
...
### Why do we need this Async [AsyncWT32_ETH01_Manager library](https://github.com/khoih-prog/AsyncWT32_ETH01_Manager)

#### Features

This is an `ESP32 + LwIP LAN8720` Credentials and Connection Manager with fallback Web ConfigPortal. This Library is used for configuring ESP32 Ethernet Static / DHCP and Credentials at runtime. You can specify static DNS servers, personalized HostName and CORS feature.

This library is based on, modified, bug-fixed and improved from:

1. [`Khoi Hoang's ESPAsync_WiFiManager`](https://github.com/khoih-prog/ESPAsync_WiFiManager)

to use the better and faster **asynchronous** [ESPAsyncWebServer](https://github.com/me-no-dev/ESPAsyncWebServer) instead of [WebServer_WT32_ETH01]((https://github.com/khoih-prog/WebServer_WT32_ETH01))

Thanks to this [AsyncWT32_ETH01_Manager library](https://github.com/khoih-prog/AsyncWT32_ETH01_Manager) is based on and sync'ed with [`ESPAsync_WiFiManager`](https://github.com/khoih-prog/ESPAsync_WiFiManager), all the features currently supported by [`ESPAsync_WiFiManager`](https://github.com/khoih-prog/ESPAsync_WiFiManager) will be available. Please have a look at [`ESPAsync_WiFiManager`](https://github.com/khoih-prog/ESPAsync_WiFiManager) for those too-many-to-list features.
...
This [**AsyncWT32_ETH01_Manager** library](https://github.com/khoih-prog/AsyncWT32_ETH01_Manager) currently supports these following boards:

 1. **WT32_ETH01 (WT32_S1 + LAN8720) boards** using `LwIP LAN8720 Ethernet`
 2. **(ESP32 + LAN8720) boards** using `LwIP LAN8720 Ethernet`
...
## Prerequisites
 ...
 7. [`WebServer_WT32_ETH01 library v1.5.1+`](https://github.com/khoih-prog/WebServer_WT32_ETH01) if necessary to use ESP32 boards using LwIP LAN8720 Ethernet. To install, check [![arduino-library-badge](https://www.ardu-badge.com/badge/WebServer_WT32_ETH01.svg?)](https://www.ardu-badge.com/WebServer_WT32_ETH01)
...
The best and easiest way is to use `Arduino Library Manager`. Search for `AsyncWT32_ETH01_Manager`, then select / install the latest version. You can also use this link [![arduino-library-badge](https://www.ardu-badge.com/badge/AsyncWT32_ETH01_Manager.svg?)](https://www.ardu-badge.com/AsyncWT32_ETH01_Manager) for more detailed instructions.

### Manual Install

1. Navigate to [AsyncWT32_ETH01_Manager](https://github.com/khoih-prog/AsyncWT32_ETH01_Manager) page.
2. Download the latest release `AsyncWT32_ETH01_Manager-main.zip`.
3. Extract the zip file to `AsyncWT32_ETH01_Manager-main` directory 
4. Copy the whole `AsyncWT32_ETH01_Manager-main` folder to Arduino libraries' directory such as `~/Arduino/libraries/`.

### VS Code & PlatformIO:

1. Install [VS Code](https://code.visualstudio.com/)
2. Install [PlatformIO](https://platformio.org/platformio-ide)
3. Install [**AsyncWT32_ETH01_Manager** library](https://registry.platformio.org/libraries/khoih-prog/AsyncWT32_ETH01_Manager) by using [Library Manager](https://registry.platformio.org/libraries/khoih-prog/AsyncWT32_ETH01_Manager/installation). Search for **AsyncWT32_ETH01_Manager** in [Platform.io Author's Libraries](https://platformio.org/lib/search?query=author:%22Khoi%20Hoang%22)
...
Thanks to [Roshan](https://github.com/solroshan) to report the issue in [Error esp_littlefs.c 'utime_p'](https://github.com/khoih-prog/AsyncWT32_ETH01_Manager/issues/28) 
...
in many files. But be sure to use the following `#include <AsyncWT32_ETH01_Manager.h>` **in just 1 `.h`, `.cpp` or `.ino` file**, which must **not be included in any other file**, to avoid `Multiple Definitions` Linker Error
...
Have a look at the discussion in [Different behaviour using the src_cpp or src_h lib #80](https://github.com/khoih-prog/AsyncWT32_ETH01_Manager/discussions/80)
...
AsyncWT32_ETH01_Manager.setSTAStaticIPConfig(WM_STA_IPconfig);
```
...
```cpp
const char * TZ_Result = AsyncWT32_ETH01_Manager.getTZ(_timezoneName);
```
...
```cpp
AsyncWT32_ETH01_Manager AsyncWT32_ETH01_Manager(&webServer, &dnsServer, "Personalized-HostName");
```
...
These illustrating steps is based on the example [Async_ConfigOnSwitchFS](https://github.com/khoih-prog/AsyncWT32_ETH01_Manager/tree/main/examples/Async_ConfigOnSwitchFS)
...
The example [Async_ConfigOnSwitchFS](https://github.com/khoih-prog/AsyncWT32_ETH01_Manager/tree/main/examples/Async_ConfigOnSwitchFS) will open the CP whenever a SW press is detected in `loop()`. So the code to add `dynamic variables` will be there, just after the CP `AsyncWT32_ETH01_Manager` class initialization to create `AsyncWT32_ETH01_Manager` object.
...
// is configuration portal requested?
  if ((digitalRead(TRIGGER_PIN) == LOW) || (digitalRead(TRIGGER_PIN2) == LOW))
  {
    ...
    AsyncWT32_ETH01_Manager AsyncWT32_ETH01_Manager(&webServer, &dnsServer, "ConfigOnSwitchFS");
...
Adding those `ESPAsync_EMParameter` objects created in Step 2 using the function `addParameter()` of object `AsyncWT32_ETH01_Manager`
...
AsyncWT32_ETH01_Manager.addParameter(&p_thingspeakApiKey);
AsyncWT32_ETH01_Manager.addParameter(&p_sensorDht22);
AsyncWT32_ETH01_Manager.addParameter(&p_pinSda);
AsyncWT32_ETH01_Manager.addParameter(&p_pinScl);
```
...
After users select `Save`, the CP `AsyncWT32_ETH01_Manager` object will save the user input data into related `ESPAsync_EMParameter` objects.
...
<p align="center">
    <img src="https://github.com/khoih-prog/AsyncWT32_ETH01_Manager/raw/main/Images/Saved.png">
</p>
...
void loop()
{
  ...
  if ((digitalRead(TRIGGER_PIN) == LOW) || (digitalRead(TRIGGER_PIN2) == LOW))
  {
    ...
    AsyncWT32_ETH01_Manager AsyncWT32_ETH01_Manager(&webServer, NULL, "ConfigOnSwitch");
#else
    AsyncDNSServer dnsServer;

    AsyncWT32_ETH01_Manager AsyncWT32_ETH01_Manager(&webServer, &dnsServer, "ConfigOnSwitch");
#endif
...
Many applications need configuration parameters like `MQTT host and port`, [Blynk](http://www.blynk.cc) or [emoncms](http://emoncms.org) tokens, etc. While it is possible to use [`AsyncWT32_ETH01_Manager`](https://github.com/khoih-prog/AsyncWT32_ETH01_Manager) to collect additional parameters, it is better to read these parameters from a web service once [`AsyncWT32_ETH01_Manager`](https://github.com/khoih-prog/AsyncWT32_ETH01_Manager) has been used to connect to the Internet.

To capture other parameters with [`AsyncWT32_ETH01_Manager`](https://github.com/khoih-prog/AsyncWT32_ETH01_Manager) is a little bit more complicated than all the other features. This requires adding custom HTML to your form. 

If you want to do it with [`AsyncWT32_ETH01_Manager`](https://github.com/khoih-prog/AsyncWT32_ETH01_Manager) see the example [Async_ConfigOnSwitchFS](examples/Async_ConfigOnSwitchFS)
...
AsyncWT32_ETH01_Manager.addParameter(&custom_text);
```
...
https://github.com/khoih-prog/AsyncWT32_ETH01_Manager/blob/b938a7f7cfa4da1efbe53a8b173ddbe7b75738ce/examples/Async_ConfigOnSwitch/Async_ConfigOnSwitch.ino#L28-L798
...
This is terminal debug output when running [Async_ConfigOnDoubleReset](examples/Async_ConfigOnDoubleReset) on  **ESP32 + LwIP LAN8720**. `Config Portal` was requested by DRD to input and save Credentials, such as Static IP address.

```
Starting Async_ConfigOnDoubleReset using LittleFS on ESP32_DEV with ETH_PHY_LAN8720
AsyncWT32_ETH01_Manager v1.0.0
...
Starting configuration portal @ 192.168.2.97
[EM] _configPortalStart millis() = 1937
[EM] Config Portal IP address = 192.168.2.97
[EM] HTTP server started
[EM] startConfigPortal : Enter loop
[EM] handleRoot
[EM] request host IP = 192.168.2.97
[EM] Info
[EM] Info page sent
[EM] handleRoot
[EM] request host IP = 192.168.2.97
...
[EM] OK
Current IP = 192.168.2.97. Reset to take new IP = 192.168.2.232
...
Local Date/Time: Thu Dec  8 21:51:18 2022
Stop doubleResetDetecting
Saving config file...
Saving config file OK
Local Date/Time: Thu Dec  8 22:34:53 2022
Local Date/Time: Thu Dec  8 22:35:53 2022
Local Date/Time: Thu Dec  8 22:36:53 2022
Local Date/Time: Thu Dec  8 22:37:53 2022
```
...
This is terminal debug output when running [Async_ConfigOnSwichFS](examples/Async_ConfigOnSwichFS) on  **ESP32 + LwIP LAN8720**. `Config Portal` was requested to input and save Credentials.


```
Starting Async_ConfigOnSwichFS using LittleFS on ESP32_DEV with ETH_PHY_LAN8720
AsyncWT32_ETH01_Manager v1.0.0
ESP_DoubleResetDetector v1.3.2
...
Config file was successfully saved
[EM] freeing allocated params!
Local Date/Time: Thu Dec  8 22:42:53 2022
Local Date/Time: Thu Dec  8 22:43:53 2022
Local Date/Time: Thu Dec  8 22:44:53 2022
```
...
This is terminal debug output when running [Async_ESP32_FSWebServer_DRD](examples/Async_ESP32_FSWebServer_DRD)  on  **ESP32 + LwIP LAN8720**. `Config Portal` was requested by DRD (also using **LittleFS**) to input and save Credentials.


```cpp
Starting Async_ESP32_FSWebServer_DRD using LittleFS on ESP32_DEV with ETH_PHY_LAN8720
AsyncWT32_ETH01_Manager v1.0.0
ESP_DoubleResetDetector v1.3.2
...
Submit issues to: [AsyncWT32_ETH01_Manager issues](https://github.com/khoih-prog/AsyncWT32_ETH01_Manager/issues)
...
- The library is licensed under [MIT](https://github.com/khoih-prog/AsyncWT32_ETH01_Manager/blob/main/LICENSE)
